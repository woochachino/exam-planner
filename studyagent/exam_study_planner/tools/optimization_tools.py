"""Schedule generation - simple round-robin distribution of topics."""
...
def generate_schedule(
    ...
    # Build interleaved topic queue (alternate subjects for variety)
    by_subject = {}
    for t in topics:
    ...
    # Interleave topics from different subjects
    ...
    while current <= end and queue_idx < len(queue) * 3:  # Safety limit
        sessions = []
        day_hours = 0
        time = 8.0
        topics_today = set()  # Track topics scheduled today

        # Fill the day with different topics
        ...
        while day_hours < max_daily and scanned < len(queue):
            ...
            # Skip if already scheduled today or done
            if topic["id"] in topics_today or topic["remaining"] < 0.25:
                continue

            # Schedule 1-1.5 hours per topic per day
            session_hours = min(1.5, topic["remaining"], max_daily - day_hours)
...
def export_schedule_markdown(tool_context: ToolContext) -> dict:
    ...
    lines.append("---")
    lines.append("*Generated by Exam Study Planner*")
